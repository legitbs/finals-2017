...
#define MULTIPART_FILE_MAXSIZE	(8192)
...
typedef struct {
   ...
   char tag_filename[HTTP_MAXNAME];	// The filename for a post request
	...
	char *multipart_file_data;
	
	int multipart_file_data_length;
...
int http_handlefile( HttpServerStruct *pServer );
