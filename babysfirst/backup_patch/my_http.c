...
_http_nodebug int http_getline(HttpServerStruct *pServer)
{
   ...
   sock_gets( &(pServer->s), pServer->input_pos, HTTP_MAX_INOUTBUFFER );
	...
	pServer->input_length = get_my_line( pServer->inout_buffer, HTTP_MAX_INOUTBUFFER-1 );
...
_http_nodebug int http_handlefile( HttpServerStruct *pServer )
{
	// Should really do this properly -- but this is a hack
	if ( pServer->multipart_file_data )
		free( pServer->multipart_file_data );

	pServer->multipart_file_data = malloc( MULTIPART_FILE_MAXSIZE );

	int fileSize = 0;

#if HTTP_VERBOSE
	printf( "http_handlefile: Handling file data!\n" );
#endif

	while ( 1  )
	{
		http_getline( pServer );

		if ( !strncmp( pServer->inout_buffer, pServer->boundary, pServer->boundary_len ) )
		{
#if HTTP_VERBOSE
			printf( "http_handlefile: Found ending boundary!\n" );
#endif	
			break;
		}

		memcpy( pServer->multipart_file_data+fileSize, pServer->inout_buffer, pServer->input_length );

		fileSize += pServer->input_length;

		if ( fileSize > MULTIPART_FILE_MAXSIZE )
		{
#if HTTP_VERBOSE
			printf( "http_handlefile: Boundary exceeded file max length!\n" );
#endif
			break;
		}
	}

	pServer->multipart_file_data_length = fileSize;
...
_http_nodebug int http_parsehead(HttpServerStruct *pServer, int part)
{
   ...
   if ( !strncmpi(pServer->inout_buffer, "Content-Type: ", 14) )
   {
      ...
      {
			pServer->has_multipart_data = TRUE;
	...
	if ( pServer->has_multipart_data && !strncmp( pServer->inout_buffer, pServer->boundary, pServer->boundary_len) ) 
	{
		return http_handlefile( pServer );
	}
   ...
   for ( iServer = 0; iServer < HTTP_MAXSERVERS; iServer++ )
   {
	...
	printf( "HTTP: http_init: Initializing server %d\n", iServer );
      ...
      // save_server_into_xmem( ppPhysicalAddressForServers, iServer, pServerBuffer );
		...
		pServer->multipart_file_data = NULL;
		pServer->multipart_file_data_length = 0;
	}
